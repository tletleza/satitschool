...
if ($teacher_id) {
    // ดึงตารางสอนของครูท่านนี้
    $stmt = $pdo->prepare("
        SELECT sc.day_of_week, sc.start_time, sc.end_time, sub.subject_code, sub.name as subject_name, 
...
<div class="container-fluid mt-4">
    <div class="d-flex justify-content-between align-items-center mb-4 text-white p-3 rounded shadow-sm" style="background: linear-gradient(135deg, #A82323, #7e1a1a);">
        <h3 class="mb-0"><i class="fas fa-chalkboard-teacher me-2"></i> ตารางสอนของฉัน</h3>
        <p class="mb-0">อาจารย์: <?= htmlspecialchars($teacher['first_name']." ".$teacher['last_name']) ?></p>
    </div>
                        ...
                        <?php if (isset($schedules[$day_en])): ?>
                            ...
                            <li class="list-group-item px-0 py-3 border-light">
                                ...
                                <div class="d-flex flex-column small text-muted">
                                    <span><i class="fas fa-door-open me-1 text-primary"></i> <?= htmlspecialchars($sch['level_name']." (".$sch['room_name'].")") ?></span>
                                    <span><i class="fas fa-map-marker-alt me-1 text-danger"></i> อาคาร <?= htmlspecialchars($sch['building']) ?> ชั้น <?= htmlspecialchars($sch['floor']) ?> ห้อง <?= htmlspecialchars($sch['room_number']) ?></span>
                                </div>
                            </li>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <li class="list-group-item px-0 py-4 text-center text-muted border-0">ไม่มีคาบสอน</li>
                        <?php endif; ?>
                    </ul>
                </div>
            </div>
