<?php
/**
 * ไฟล์ teachers.php: ระบบจัดการข้อมูลครูผู้สอน (CRUD)
 * สำหรับ Admin เท่านั้น ใช้ในการ เพิ่ม, แก้ไข, ลบ และแสดงรายชื่อครูทั้งหมดในโรงเรียน
 */
require_once 'includes/check_auth.php';
require_once 'includes/check_role.php';
authorize(['admin']); // ตรวจสอบสิทธิ์ (เฉพาะ Admin)
require_once 'includes/db.php';

// ส่วนที่ 1: การอัปเดตโครงสร้างฐานข้อมูลอัตโนมัติ (Schema Update)
// ป้องกันความผิดพลาดหากฐานข้อมูลยังไม่มีคอลัมน์ที่จำเป็น
try {
    $pdo->query("SELECT teacher_code FROM teachers LIMIT 1");
} catch(Exception $e) {
    // เพิ่มคอลัมน์ รหัสครู และ หมวดวิชา เข้าไปในตาราง teachers
