<?php
/**
 * ไฟล์ classrooms.php: ระบบจัดการห้องเรียนและระดับชั้น
 * ใช้สำหรับกำหนดชื่อห้อง, ระดับชั้น (ม.1 - ม.6) และที่ตั้งอาคาร/ชั้น/เลขที่ห้อง
 */
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_classroom'])) {
    ...
    if ($id) {
        $stmt = $pdo->prepare("UPDATE classrooms SET grade_id=?, room_name=?, building=?, floor=?, room_number=?, advisor_id=? WHERE id=?");
        $stmt->execute([$grade_id, $room_name, $building, $floor, $room_number, $advisor_id, $id]);
        $msg = "แก้ไขข้อมูลห้องเรียนสำเร็จ!";
    } else {
        $stmt = $pdo->prepare("INSERT INTO classrooms (grade_id, room_name, building, floor, room_number, advisor_id, academic_year_id) VALUES (?, ?, ?, ?, ?, ?, 1)");
        $stmt->execute([$grade_id, $room_name, $building, $floor, $room_number, $advisor_id]);
                ...
                <table class="table table-hover align-middle">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><strong><?= htmlspecialchars($c['building'] ?: '-') ?></strong></td>
                            <td><?= htmlspecialchars($c['floor'] ?: '-') ?></td>
                            <td><span class="badge bg-light text-dark border"><?= htmlspecialchars($c['room_number'] ?: '-') ?></span></td>
      ...
      <div class="modal-body">
        ...
        <div class="row mb-3">
            <div class="col-md-4">
                ...
                <input type="text" name="building" id="c_building" class="form-control" placeholder="เช่น 6" required>
            </div>
            <div class="col-md-4">
                <label class="form-label fw-bold">ชั้น</label>
                <input type="text" name="floor" id="c_floor" class="form-control" placeholder="เช่น 2" required>
            </div>
            <div class="col-md-4">
                <label class="form-label fw-bold">เลขที่ห้อง</label>
                <input type="text" name="room_number" id="c_room_num" class="form-control" placeholder="เช่น 4" required>
            </div>
        </div>
        <div class="mb-3">
...
function clearForm() {
    ...
    document.getElementById('c_building').value = '';
    document.getElementById('c_floor').value = '';
    document.getElementById('c_room_num').value = '';
...
function editClassroom(c) {
    ...
    document.getElementById('c_building').value = c.building || '';
    document.getElementById('c_floor').value = c.floor || '';
    document.getElementById('c_room_num').value = c.room_number || '';
